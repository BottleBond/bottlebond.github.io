...
**Created**: 2026-01-29  
**Status**: In Progress  
**Input**: Modern podcast website for Bourbon & Whisky-focused audio/video content with luxury lounge aesthetic; replacing https://bottle.bond
...
2. **Given** posts display, **When** era tags present, **Then** posts group/filter by era hashtag (formatting TBD)
...
- **FR-010**: System MUST order Glass Room posts by era hashtag or publication date (oldest first, configurable)
...
- **Episode**: Title, description, YouTube video ID, playlist category (main, education, tastings), popularity score, thumbnail URL, duration
- **Person**: Name, role (host, cohost, guest), bio, photo URL, social links (optional)
- **BlogPost**: Title, content, era hashtag(s), publication date, author, last edited date, status (draft/published)
- **Playlist**: Name, YouTube ID, description, category (main, education, tastings)
...
- **Deployment**: Static site generation (Next.js, Astro, Hugo, etc.) with build-time markdown parsing for blog posts
